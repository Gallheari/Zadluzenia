...
function Dashboard() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const debtsList = debtsSnapshot.docs.map(doc => ({ id: doc.id, ...doc.data(), remainingAmount: doc.data().remainingAmount ?? doc.data().amount }));
        setDebts(debtsList);
  ...
  const barChartData = [
    { name: 'Długi', 'Całkowite Zadłużenie': totalDebtAmount, 'Pozostało do Spłaty': totalRemainingAmount },
  ];

  return (
    <Box>
      <Typography variant="h4" gutterBottom component="div">Panel główny</Typography>
      {currentUser && <Typography variant="h6" gutterBottom>Witaj, {currentUser.name}!</Typography>}

      {loading ? (
        <CircularProgress />
      ) : !currentUser ? (
        <Typography>Wybierz lub dodaj użytkownika, aby zarządzać długami.</Typography>
      ) : (
